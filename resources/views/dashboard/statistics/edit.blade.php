                        ...
                        <form method="POST" action="{{ route('admin.statistics.update', $statistic->id) }}"
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                                    :value="old('name', $statistic->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="value" :value="__('Value')" />
                                <x-text-input id="value" class="block mt-1 w-full" type="number" min="0"
                                    name="value" :value="old('value', $statistic->value)" required autofocus />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="image"
                                        ...
                                        name="image" :value="old('image', $statistic->image->path ?? '')"/>
                                    <x-input-error :messages="$errors->get('image')" class="mt-2" />
                                </div>
                                @isset($statistic->image->path)
                                    <div class="-mt-2">
                                        <img src="{{ asset($statistic->image->path) }}" alt="{{ $statistic->name }}"
